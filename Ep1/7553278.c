...
void inserirPrefQ(PFILA f, PONT a){
	a->prox = f->inicioNaoPref;
  	a->ant = f->inicioNaoPref->ant;
  	f->inicioNaoPref->ant->prox = f->inicioNaoPref->ant = a;
}

void inserir2(PFILA f, PONT a){
	f->inicioNaoPref = a;
	f->cabeca->prox->prox = a;
	f->cabeca->ant = a;
	a->ant = f->cabeca->prox;
	a->prox = f->cabeca;
}

void inserir3(PFILA f, PONT a){
	a->prox = f->cabeca;
    a->ant = f->cabeca->ant;
    f->cabeca->ant->prox = a;
    f->cabeca->ant = a;
...
//teste de entrada
  	...
  	if(ehPreferencial==1)
  {
  	...
  	inserirPrefQ(f, insert);
  	return true;
  }
}
  	else if(ehPreferencial == 0){

  	if(f->inicioNaoPref->id == -1)//primeiro nao preferencial numa fila com preferenciais
  	{	
  		inserir2(f, insert);
			return true;
  	}
  	else //enesimo nao preferencial
  		{
  			inserir3(f, insert);
			return true;
				}
